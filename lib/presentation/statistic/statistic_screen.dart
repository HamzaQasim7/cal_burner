...
//         break;
//       case 2:
//         activityProvider.loadYearlyActivities();
//         break;
//     }
//   }
//
...
class _StatisticsScreenState extends State<StatisticsScreen> {
                      ...
                      child: Column(
                        children: [
                              ...
                              Flexible(
                                child: StatCardWidget(
                                  title:
                                      'statistics.weekly_average_distance'.tr(),
                                  value:
                                      '${((monthlySummary['totalDistance'] ?? 0) / (monthlySummary['daysInMonth'] ?? 1)).toStringAsFixed(1)} km',
                                ),
                              ),
                            ],
                          ),
                          ...
                          _buildChartCard(
                            title: 'statistics.exercise'.tr(),
                            value:
                                '${weeklySummary.totalCalories.toStringAsFixed(0)} kcal',
                            subtitle: 'statistics.past_5_days'.tr(),
                            chartWidget: _buildExerciseChart(activityProvider),
                          ),
  ...
  Widget _buildStepsChart(ActivityProvider provider) {
    final theme = Theme.of(context);
    final isDark = theme.brightness == Brightness.dark;
    final activities = provider.weeklyActivities;

    return BarChart(
      BarChartData(
        alignment: BarChartAlignment.spaceAround,
        maxY: _getMaxSteps(activities),
        barTouchData: BarTouchData(
          enabled: true,
          touchTooltipData: BarTouchTooltipData(
            getTooltipItem: (group, groupIndex, rod, rodIndex) {
        ...
        titlesData: _buildChartTitles(context),
        borderData: FlBorderData(show: false),
        gridData: FlGridData(show: false),
        barGroups: _generateStepBarGroups(activities, isDark, theme),
      ),
    );
  }

  Widget _buildExerciseChart(ActivityProvider provider) {
    final theme = Theme.of(context);
    final isDark = theme.brightness == Brightness.dark;
    final activities = provider.weeklyActivities;
    ...
    return BarChart(
      BarChartData(
        alignment: BarChartAlignment.spaceAround,
        maxY: _getMaxCalories(activities),
        barTouchData: BarTouchData(
          enabled: true,
          touchTooltipData: BarTouchTooltipData(
            getTooltipItem: (group, groupIndex, rod, rodIndex) {
        ...
        borderData: FlBorderData(show: false),
        gridData: FlGridData(show: false),
        barGroups: _generateCalorieBarGroups(activities, isDark, theme),
      ),
    );
  }

  FlTitlesData _buildChartTitles(BuildContext context) {
    final theme = Theme.of(context);
    final today = DateTime.now();
    final days = List.generate(5, (index) {
      final day = today.subtract(Duration(days: 4 - index));
      return DateFormat('EEE').format(day).tr(); // Format as short weekday name
    });
